    ...
    fn parse_statement(pair: Pair<Rule>, context: &mut HashSet<Literal>) -> (Vec<Literal>, Vec<Expression>) {
        match pair.as_rule() {
            Rule::inner_number | Rule::number => {
                let name = pair.into_inner().next().unwrap().as_str().parse::<isize>().unwrap();
            ...
            Rule::relation_name | Rule::lower_word | Rule::function_name | Rule::formula_name => unreachable!(),
            Rule::variable_name => {
            ...
            Rule::constant_declaration => {
                ...
                let new_expression = Expression::BasicEquality(new_constant.clone(), new_integer.clone());
            ...
            Rule::function_statement => {
                ...
                for literal in context.iter() {
                    if literal.name() == name && literal.arity() == literals.len() && literal.typ() == Type::FuncName {
                    ...
                    } else if literal.name() == name && literal.arity() != literals.len() && literal.typ() == Type::FuncName {
            ...
            Rule::equality_statement => {
                ...
                let new_expression = Expression::nequality(literals1.get(0).unwrap(), literals2.get(0).unwrap());
            ...
            Rule::relation_statement => {
                ...
                for literal in context.iter() {
                    if literal.typ() == Type::RelName && literal.name() == name && literal.arity() == terms.len() {
                    ...
                    } else if literal.typ() == Type::RelName && literal.name() == name && literal.arity() != terms.len() {
            ...
            Rule::equivalent_expression => {
                ...
                if let Some(inner_pair) = inners.next() {
                    ...
                    let new_expression = Expression::nequivalent(expr1[0].clone(), expr2[0].clone());
